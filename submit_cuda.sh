...
case "${MODE}" in
    compare)
        EXECUTABLE="install/bin/cooling_cuda_compare"
        HDF5_OUTPUT="output/Cooling_cuda.h5"
        CSV_OUTPUT="output/Cooling_cuda.csv"
        OUTPUT_EVERY=20
        ;;
    ...
    benchmark)
        ...
        HDF5_OUTPUT="none"
        CSV_OUTPUT="output/Cooling_cuda_bench.csv"
        OUTPUT_EVERY=0
        ;;

    *)
        echo "[ERROR] Mode not valid: ${MODE}" >&2
        echo "Use: sbatch submit_cuda.sh {compare|benchmark}" >&2
        exit 2
        ;;
esac
