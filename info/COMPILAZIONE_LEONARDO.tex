...
Iniziare da un ambiente riproducibile e caricare NVHPC, che mette a
disposizione sia i compilatori HPC sia il toolchain CUDA:
...
Questo profilo serve a confrontare i campi prodotti dalle cinque
implementazioni. Tutti gli eseguibili:

\begin{itemize}
  \item usano \texttt{nvc++} per C++/OpenMP/OpenACC e \texttt{nvcc} per
  CUDA C++;
...
\subsection{GPU CUDA A100 con HDF5}
...
nvcc -O3 -std=c++17 \
  ...
  -DUSE_HDF5=1 \
  -I"${HDF5_INCLUDE}" \
  src/cpp/cooling_cuda.cpp \
  src/cpp/cuda/kernels.cu \
  -o install/bin/cooling_cuda_compare \
  -L"${HDF5_LIB}" \
  -Xlinker=-rpath,"${HDF5_LIB}" \
  -lhdf5_cpp -lhdf5 -lz -ldl -lm -lstdc++fs
\end{lstlisting}

Anche per CUDA la contrazione FMA viene disabilitata esplicitamente: il flag
\texttt{--fmad=false} agisce sul codice dispositivo, mentre le opzioni passate
con \texttt{-Xcompiler} rendono conservativa la compilazione del codice host.
\texttt{nvcc} collega automaticamente il runtime CUDA; \texttt{-Xlinker}
inoltra invece il percorso runtime HDF5 al linker host.
...
\subsection{GPU CUDA A100 ottimizzata}
...
nvcc -O3 -std=c++17 \
  ...
  src/cpp/cooling_cuda.cpp \
  src/cpp/cuda/kernels.cu \
  -o install/bin/cooling_cuda_bench \
  -lstdc++fs
\end{lstlisting}
...
\begin{description}
  ...
  \item[\texttt{-arch=sm\_80}] è l'equivalente \texttt{nvcc} che genera
  codice CUDA per le GPU NVIDIA A100;
...
\begin{description}
  ...
  codice dispositivo;
  \item[\texttt{--fmad=false}] disabilita la contrazione FMA nel codice
  dispositivo compilato con \texttt{nvcc};
  \item[\texttt{-Xcompiler=-fno-fast-math,-ffp-contract=off}] inoltra al
  compilatore host opzioni floating-point conservative;
  ...
  \item[\texttt{-Wl,-rpath,...}] registra nell'eseguibile compilato con
  \texttt{nvc++} il percorso delle librerie HDF5, così il loader può
  ritrovarle durante il job;
  \item[\texttt{-Xlinker=-rpath,...}] realizza la stessa operazione nei
  comandi \texttt{nvcc}, inoltrando le opzioni al linker host;
...
\begin{description}
  ...
  validator;
  \item[\texttt{--use\_fast\_math}] è l'opzione equivalente usata da
  \texttt{nvcc} nel benchmark CUDA.
\end{description}
