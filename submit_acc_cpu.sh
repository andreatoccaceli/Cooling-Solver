...
case "${MODE}" in
    compare)
        EXECUTABLE="install/bin/cooling_acc_cpu_compare"
        HDF5_OUTPUT="output/Cooling_acc_cpu.h5"
        CSV_OUTPUT="output/Cooling_acc_cpu.csv"
        OUTPUT_EVERY=20
        ;;
    ...
    benchmark)
        ...
        HDF5_OUTPUT="none"
        CSV_OUTPUT="output/Cooling_acc_cpu_bench.csv"
        OUTPUT_EVERY=0
        ;;

    *)
        echo "[ERROR] Mode not valid: ${MODE}" >&2
        echo "Use: sbatch submit_acc_cpu.sh {compare|benchmark}" >&2
        exit 2
        ;;
esac
