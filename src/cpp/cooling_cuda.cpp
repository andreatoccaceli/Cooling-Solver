...
void ensureParentDirectoryExists(const std::string& fileName) {
    if (fileName.empty() || isNoHdf5Token(fileName)) return;
    ...
    if (fs::exists(parent, ec)) {
        if (!fs::is_directory(parent, ec)) {
            throw std::runtime_error(
                "Output parent exists but is not a directory: " + parent.string());
        }
        return;
    }
...
SimulationConfig readConfigurationFile(const std::string& fileName) {
    ...
    auto nextDouble = [&]() -> double {
        ...
        try {
            ...
            throw std::runtime_error(
                "Malformed input: invalid floating-point token '" + token + "'");
        }
    };
    ...
    if (pos != tokens.size()) {
        throw std::runtime_error(
            "Malformed input: unexpected extra tokens at end of file");
    }
...
double computeMeanDiscrepancy(const SimulationConfig& cfg) {
    if (cfg.measuredPoints.empty()) return 0.0;
    double sum = 0.0;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help" || arg == "-h") {
            std::cout
                << "Usage:\n"
                << "  " << argv[0]
                << " [inputFile] [h5File|none|--no-hdf5] [csvFile] [outputEvery]\n\n";
            std::exit(0);
        }
...
bool shouldWriteStep(int step, int finalStep, int outputEvery) {
    ...
    if (outputEvery <= 0)  return false;
    if (step == 0)         return true;
    return (step % outputEvery) == 0;
}

void printRunHeader(const CommandLineOptions& cli,
                    const SimulationConfig& cfg,
                    bool hdf5Compiled) {
    std::cout << "Input file:                    " << cli.inputFile << '\n';
    std::cout << "CSV output:                    " << cli.csvFile << '\n';
    std::cout << "HDF5 compiled:                 " << (hdf5Compiled ? "yes" : "no") << '\n';
    std::cout << "HDF5 output:                   "
              << (cli.writeHdf5 ? cli.h5File : "disabled") << '\n';
    std::cout << "Grid:                          "
              << cfg.gridWidth << " x " << cfg.gridHeight << '\n';
...
// -------------------------------------------------------------------------
// Optional HDF5 writer
// -------------------------------------------------------------------------
#ifdef USE_HDF5
class TimeSeriesWriter {
public:
    TimeSeriesWriter(const std::string& fileName,
                     std::size_t width,
                     std::size_t height,
                     std::size_t batch  = 32,
        ...
        : file_(fileName, H5F_ACC_TRUNC),
          ...
          batch_(batch), frameCount_(0), capacity_(batch), closed_(false)
    {
...

        {
            ...
            hsize_t maxdims[3] = {H5S_UNLIMITED,
            ...
            fieldDataset_ = file_.createDataSet(
                "/field", H5::PredType::NATIVE_DOUBLE, space, props);
        }

        {
            ...
            H5::DSetCreatPropList props1;
            hsize_t chunks1[1] = {static_cast<hsize_t>(batch_)};
            props1.setChunk(1, chunks1);

            stepDataset_ = file_.createDataSet(
                "/step", H5::PredType::NATIVE_INT, space1, props1);
        }
    ...
    void writeFrame(int stepNumber, const std::vector<double>& field) {
...

        {
            ...
            fieldDataset_.write(
                field.data(), H5::PredType::NATIVE_DOUBLE, memspace, filespace);
        }

        {
            H5::DataSpace filespaceStep = stepDataset_.getSpace();
            hsize_t startStep[1] = {static_cast<hsize_t>(frameCount_)};
            hsize_t countStep[1] = {1};
            filespaceStep.selectHyperslab(H5S_SELECT_SET, countStep, startStep);
            H5::DataSpace memspaceStep(1, countStep);
            stepDataset_.write(
                &stepNumber, H5::PredType::NATIVE_INT, memspaceStep, filespaceStep);
        }
    ...
    void close() {
        if (closed_) return;
        if (frameCount_ != capacity_) extend(frameCount_);
...
private:
    void extend(std::size_t newSize) {
        hsize_t fieldDims[3] = {static_cast<hsize_t>(newSize),
                                static_cast<hsize_t>(height_),
                                static_cast<hsize_t>(width_)};
    ...
    std::size_t width_{}, height_{}, batch_{}, frameCount_{}, capacity_{};
    bool closed_{false};
};
#else
class TimeSeriesWriter {
public:
    TimeSeriesWriter(const std::string&,
                     std::size_t,
                     std::size_t,
                     std::size_t = 32,
                     std::size_t = 256,
                     std::size_t = 256)
    {
        throw std::runtime_error("HDF5 not enabled.");
    }
